...
if ($search) {
    ...
    $conditions[] = "(u.name LIKE '%$search_esc%' OR d.name LIKE '%$search_esc%' OR a.appointment_date LIKE '%$search_esc%')";
}
...
<form method="GET">
    <input type="text" name="search" placeholder="Search patient, doctor or date" value="<?php echo htmlspecialchars($search); ?>">
...
for ($i = 1; $i <= $total_pages; $i++) {
    if ($i == $page) {
        ...
        echo '<a href="admin_dashboard.php?page='.$i.'&search='.urlencode($search).'&'.http_build_query(['status'=>$status_filter]).'">'.$i.'</a> ';
    }
}
?>
</p>
